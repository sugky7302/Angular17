#!/usr/bin/env bash
set -Eeo pipefail
# 複製自 https://github.com/docker-library/postgres/blob/master/docker-entrypoint.sh
...
_main() {
	...
	if [ "$1" = 'postgres' ] && ! _pg_want_help "$@"; then
		...
		#* 因為這裡的 docker_temp_server_start 會啟動 postgresql 並且不會監聽外部連線，
		#* 所以這裡的 postgresql 並不是真正的 postgresql
		#* 我希望 postgresql 每次建構時都會執行 create.sql
		#* 因此我從 postgresql 的 docker-entrypoint.sh 複製了一份出來，
		#* 並且修改了 docker_temp_server_start 的部分 - 2023-02-22
